...
layout (push_constant) uniform push_constants
{
  ...
  float u_Time;
  float u_HeightScale;
  float u_HeightOffset;
  mat4 u_modelview;
  mat4 u_projection;
} pc;

vec3 iResolution = vec3(pc.u_Resolution, 1.);
float iTime = pc.u_Time;

// Based on https://www.shadertoy.com/view/Xsf3zX
...
vec3 cameraPos = inverse(pc.u_modelview) * vec4(0, 0, 0, 1);
...
vec2 Terrain( in vec2 p)
{
	...
	return vec2(f * pc.u_HeightScale, type);
}
...
void main()
{
  ...
  vec4 viewSpaceDir = inverse(pc.u_projection) * vec4(clipSpaceCoord.x, clipSpaceCoord.y, 1, 1);
  vec4 worldSpaceDir = inverse(pc.u_modelview) * vec4(normalize(viewSpaceDir.xyz), 0);

	mat3 camMat = mat3(inverse(pc.u_modelview));
  vec3 dir = worldSpaceDir; //camMat * vec3(0, 0, 1);
