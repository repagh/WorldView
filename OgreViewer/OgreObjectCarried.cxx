...
void OgreObjectCarried::init(Ogre::SceneManager *manager)
{
  try {
    Ogre::MeshPtr m =
      Ogre::MeshManager::getSingleton().load(mesh_name, groupname);
    entity = manager->createEntity(name.c_str(), mesh_name.c_str());
    node =
      manager->getSceneNode("__ego_node__")->createChildSceneNode(name.c_str());
...
void OgreObjectCarried::iterate(TimeTickType ts, const BaseObjectMotion &base,
                                double late)
{
...
static SubContractor<OgreObjectTypeKey,
                     OgreObjectCarried> *OgreObjectCarried_maker =
  new SubContractor<OgreObjectTypeKey, OgreObjectCarried>("OgreObjectCarried");
