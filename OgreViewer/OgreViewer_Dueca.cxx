...
const ParameterTable *OgreViewer_Dueca::getParameterTable()
{
  static const ParameterTable parameter_table[] = {
    ...
    { "set-x-display",
      new MemberCall<_ThisObject_, string>(&_ThisObject_::setXDisplay),
    ...
    { "viewport-overlay",
      new MemberCall<_ThisObject_, string>(&_ThisObject_::setViewportOverlay),
      "Specify an overlay for this viewport" },

    { "load-scene",
      new MemberCall<_ThisObject_, vector<string>>(&_ThisObject_::addScene),
    ...
    { "sync-divisor",
      new VarProbe<_ThisObject_, int>(&_ThisObject_::glx_sync_divisor),
      "For video sync wait mode, set the divisor (e.g. 2 means 30 Hz at a\n"
      "60 Hz refresh rate)" },

    { "sync-offset",
      new VarProbe<_ThisObject_, int>(&_ThisObject_::glx_sync_offset),
    ...
    { "render-shadows",
      new MemberCall<_ThisObject_, bool>(&_ThisObject_::addShadows),
    ...
    { "set-shadow-technique",
      new MemberCall<_ThisObject_, string>(&_ThisObject_::setShadowRenderer),
    ...
    { "set-shadow-far-distance",
      new MemberCall<_ThisObject_, float>(&_ThisObject_::setShadowFarDistance),
...
bool OgreViewer_Dueca::addObjectClass(const std::vector<std::string> &names)
{
  ...
  OgreObjectCompatible::typemap[names[0]] =
    (names.size() == 2)
...
bool OgreViewer_Dueca::addResourceLocation(const std::vector<std::string> &sc)
{
  ...
  resourcespec.push_back(ResourceSpec(
    sc[0], sc[1], sc[2], sc.size() == 4 && (sc[3] == "recursive")));
  return true;
}
...
{
  if ((render_technique == "stencil") || (render_technique == "Stencil") ||
      (render_technique == "STENCIL")) {
    build_view_spec.shadow_technique = "stencil";
  }
  else if ((render_technique == "texture") || (render_technique == "Texture") ||
           (render_technique == "TEXTURE")) {
    build_view_spec.shadow_technique = "texture";
  }
  else {
    E_CNF("set-shadow-technique unknown shadow technique. Choices are Texture "
          "or Stencil");
  }
  return true;
}
...
{
  if (colour.size() != 3) {
    E_CNF("Wrong number of arguments for set-shadow-colour. Requires three "
          "arguments, one for each (r,g,b) colour value!");
  }
  else {
...
{
  if (distance < 0) {
    E_CNF("Invalid shadow far distance. set-shadow-far-distance needs to be >= "
          "zero!");
  }
  else {
