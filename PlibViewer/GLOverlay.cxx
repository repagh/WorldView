...
const static double def_vpoints[8] = { 0.0, 0.0, 1.0, 0.0, 1.0, 1.0, 0.0, 1.0 };
  ...
  viewno(0)
{
  ...
  if (spec.coordinates.size() == 9) {
    vector<double>::const_iterator jj = spec.coordinates.begin();
    jj++;
...
void GLOverlay::draw(int viewno)
{
  ...
  if (vpoints.size()) {
    ...
    glVertex2d(vpoints[0], vpoints[1]);
    glTexCoord2f(1.0f, 0.0f);
    glVertex2d(vpoints[2], vpoints[3]);
    glTexCoord2f(1.0f, 1.0f);
    glVertex2d(vpoints[4], vpoints[5]);
    glTexCoord2f(0.0f, 1.0f);
    glVertex2d(vpoints[6], vpoints[7]);
  }
  else {
    glTexCoord2f(0.0f, 0.0f);
    glVertex2d(0.0, 0.0);
...
void GLOverlay::iterate(TimeTickType ts, const BaseObjectMotion &base,
                        double late)
{
  // ??
}
...
static SubContractor<PlibObjectTypeKey, GLOverlay> *GLOverlay_maker =
  new SubContractor<PlibObjectTypeKey, GLOverlay>("overlay");
